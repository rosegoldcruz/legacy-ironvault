    ...
    <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, zIndex: 1000, background: '#F5C518', padding: '10px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 8, borderTop: '2px solid #B8860B' }}>
      ...
      <nav style={{ display: 'flex', alignItems: 'center', gap: 4, flexWrap: 'wrap' }}>
        {links.map((link) => {
          ...
          return (
            <a key={link.label} href={link.href} style={{ fontFamily: 'Montserrat, sans-serif', fontSize: 11, fontWeight: 600, color: active ? '#F5C518' : '#000', textDecoration: 'none', padding: active ? '4px 10px' : '4px 8px', background: active ? '#000' : 'transparent', borderRadius: active ? 3 : 0 }}>
              {link.label}
            </a>
          )
        })}
